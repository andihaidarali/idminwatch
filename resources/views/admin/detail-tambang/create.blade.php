<x-admin-layout title="Tambah Perusahaan - Admin">
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-white">Tambah Perusahaan</h1>
        <p class="text-sm text-gray-400 mt-1">Buat master perusahaan. Wilayah tambang dapat dihubungkan ke perusahaan ini dari halaman edit wilayah.</p>
    </div>

    <form action="{{ route('detail-tambang.store') }}" method="POST" class="space-y-8 w-full max-w-5xl">
        @csrf
                ...
                <div class="grid grid-cols-1 xl:[grid-template-columns:minmax(0,1fr)_minmax(0,1fr)] gap-5 items-start">
                    <div class="min-w-0 overflow-hidden">
                        ...
                        <textarea name="profil_singkat" id="profil_singkat" rows="6"
                            class="rich-editor w-full px-4 py-3 bg-gray-800/50 border border-gray-700/50 rounded-xl text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 resize-none">{{ old('profil_singkat') }}</textarea>
                    </div>
                    <div class="min-w-0 overflow-hidden">
                        ...
                        <textarea name="profil_singkat_en" id="profil_singkat_en" rows="6"
                            class="rich-editor w-full px-4 py-3 bg-gray-800/50 border border-gray-700/50 rounded-xl text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 resize-none">{{ old('profil_singkat_en') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
